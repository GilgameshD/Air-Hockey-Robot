...
void positionControl()
{
  ...
  temp = temp/(1800*(long)acceleration_x);  // 2000*0.85 = 1700 0.85 is a compensation for deceleration ramp (S-curve) 对S型曲线的补偿
  pos_stop_x = position_x + sign(speed_x)*temp;
  if (target_position_x > position_x)  // Positive move
  {
    if (pos_stop_x >= target_position_x)  // 开始减速
...
{ 
  ...
  if (speed_x==0)
    ...
    timer_period = TIMER_PARAMETER_X/speed_x;   // 2Mhz timer
  else
    timer_period = TIMER_PARAMETER_X/-speed_x;
...
{ 
  ...
  if (speed_y == 0)
    ...
    timer_period = TIMER_PARAMETER_Y/speed_y;   // 2Mhz timer
  else
    timer_period = TIMER_PARAMETER_Y/-speed_y;
